...
#---------------------------------
 ...
 readonly test_id=`date "+%Y%m%d-%H%M%S"`      # create a new time tag
#readonly test_id="20230224-171857"            # use an old time tag 

# Code to be cloned and tested

readonly code_repo="git@github.com:E3SM-Project/v3atm.git"

readonly branch_shortname="aerosol-process-coupling-202302"
readonly branch_longname="huiwanpnnl/atm/"${branch_shortname}

readonly master_hash="0a642b7"

# Test suites to be run

std_testsuite="e3sm_atm_developer"  # Script will do baseline comparison for this suite
new_testsuite="e3sm_atm_stealth"    # Script will run this suite w/o baseline comparison
